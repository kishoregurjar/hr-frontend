...
const LoginForm = () => {
  ...
  const [loginRole, setLoginRole] = useState("HR"); // "HR" | "ADMIN"
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const userName =
        ...
        (loginRole === "ADMIN" ? "Platform Admin" : "HR Recruiter");

      toast.success(`Welcome back, ${userName}!`, {
        description:
          loginRole === "ADMIN"
            ? "Signed in to Super Admin Console."
            : "Signed in to HR Recruiter Portal.",
      });
      ...
      if (isSuperAdmin && loginRole === "ADMIN") {
        router.push("/admin");
      } else if (isSuperAdmin && email.toLowerCase().includes("admin")) {
        router.push("/admin");
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 font-sans">
      {/* ── Role Selector Tabs ── */}
      <div className="flex items-center gap-1.5 p-1 bg-slate-100/90 rounded-xl border border-slate-200/70 mb-2">
        <button
          type="button"
      ...
      <div className="space-y-1.5">
        <Label htmlFor="email" className="text-xs font-bold text-slate-700 uppercase tracking-wider">
          {loginRole === "ADMIN" ? "Admin Email" : "Work Email"}
        </Label>
        <div className="relative">
          ...
          <Input
            ...
            type="email"
            placeholder={
              loginRole === "ADMIN" ? "admin@mindersworld.com" : "recruiter@company.com"
            }
            value={email}
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative">
          ...
          <button
            ...
            className="absolute right-3.5 top-3 text-slate-400 hover:text-slate-700 focus:outline-none transition-colors cursor-pointer"
            tabIndex={-1}
      ...
      >
        {isSubmitting ? (
          ...
          <span className="flex items-center justify-center gap-1.5">
            {loginRole === "ADMIN" ? "Sign In to Super Admin" : "Sign In to HR Dashboard"}
